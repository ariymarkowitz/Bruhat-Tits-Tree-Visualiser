...
Another way of viewing at the tree is as a way of building a p-adic expansion. Each vertex v contains a unique lattice L generated by vectors (1, u) and (0, p^n) for some u in Qp, n in Z, and since (1, u) can be considered as the projective point corresponding to u, L can be considered to be u up to addition by (p-adic) integer multiples of p^n; in other words, u 'mod p^n'. Hence v can be identified with some p-adic expansion
